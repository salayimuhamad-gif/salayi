...
| `tests/Feature/MapCompareTest.php` | Map Phase 6 `GET /map/compare`: flag gating; 2–3 bounds (1/4+/duplicates → 422, `distinct` errors); submitted order preserved; unknown/unpublished/draft-ancestry slugs → one undisclosing 404; exact area-row keys (no WKT/phone); services from AreaServiceSummary with `feature_disabled` ≠ zero; price rows asserted BYTE-EQUAL to `/location/resolve` for the same area (the extraction regression), ancestor fallback naming the ancestor and `shared_source` suppressing a fabricated 0% between siblings; sale/rent + property-type isolation; compatible evidence → movement + price facts with server-computed Decimal figures (52.07%/3.0000 pinned); asking-vs-verified, currency, basis and methodology mismatches each refused with their named reason; single observation = price without movement (never flat); `market.intelligence` off = `feature_disabled` never `insufficient`; own `map-compare` 429 budget. Both CI database lanes. |
| `tests/Feature/MapSearchTest.php` | Map Phase 5 `GET /map/search`: flag gating (404 off), 2–80 length bounds (422), a query folding to noise answers honest empty groups; Sorani/Arabic/English/alias and keyboard-variant Sorani spellings all find the entity through the ONE `search_key`; `%`/`_` can never act as wildcards; the three row shapes are exact key lists (no WKT, no phone anywhere in the payload); every exclusion gate (unpublished area/project/place, draft-parent ancestry, null-coords project, non-public/duplicate-secondary/closed/low-confidence places, `places.database` off); rank beats alphabetics (exact > name-prefix > contains, alias-exact outranks name-contains); caps 5/5/7; query echo. Runs on both CI database lanes. |
...
| `map-compare.spec.ts` | Map Phase 6 on `/map`: Compare mode builds the set through the Phase 5 search (areas-only dropdown proven against a query that offers a project in Explore); slot A/B/C chips; the fit-to-compared camera proven by the z≥13 `/map/features` request bracketing the seeded ring; Ankawa's +5.04% all-window claim beside Kasnazan's honest "not enough evidence"; education 1 vs "0 recorded" from the SAME service counts; apartment-filtered Kasnazan/Baharka = compatible evidence → diverged-movement + 52.07% price facts, with window/transaction changes preserving both slots; a fourth or duplicate area refused in words; `browser-dinar`'s IQD single value vs Ankawa's USD refused with "the currencies differ" (both figures shown, no conversion, no ranking); phones stack A/B cards with no sideways scroll and the map one tap away; an Arabic pass renders the grid RTL. Fixture: `browser-dinar` (published, no coords/boundary, ONE IQD value — no mover, no heat row, no pin). |
