...
| Spec | Pins |
|---|---|
| `map-production.spec.ts` | The working-style path (serves a deterministic zero-network style for `demotiles.maplibre.org/**`): homepage map becomes a real painted canvas in ckb/ar/en; canvas `position:absolute` regression; `/map` loading veil leaves; honest failure + live list when style unavailable; all four trend semantics on `/invest` markers from the real features response; marker click on the actual canvas selects the project; admin picker recovers from `v-show`-hidden tab; click-to-place yields Erbil-plausible coords; mobile map/list tab switch with no rebuild/no loader. |
