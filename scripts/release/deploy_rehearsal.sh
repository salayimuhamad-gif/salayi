...
# upgrade of a running site. This release adds exactly ELEVEN migrations above
...
# WhatsApp verification table with its users column, and the valuation rule
# engine's five tables with their four additive valuation columns), and
# "exactly eleven" can only be demonstrated against a database that is
# already migrated to the baseline.
...
echo "== 8. migrations (this patch adds exactly eleven) =="
...
# This release ships ELEVEN forward-only migrations above the v6 baseline:
...
# historical imported prices visible to the scoped market indices, and
# the Wave 6 valuation rule engine (five additive rule/answer/snapshot
# tables plus four nullable portfolio_valuations columns).
...
[ "$DELTA" = "11" ]
check "exactly eleven migrations applied ($BEFORE -> $AFTER)" $?

# Then prove each named migration is the one that ran — a count alone cannot
# tell eleven expected arrivals apart from ten expected and one stranger.
for migration in \
    ...
    2026_08_22_000100_valuation_rule_engine; do
