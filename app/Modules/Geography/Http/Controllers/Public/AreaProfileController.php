...
final class AreaProfileController extends Controller
{
    ...
    public function show(Request $request, string $slug): Response
    {
        ...
        return Inertia::render('Public/Areas/Show', [
            ...
            /*
             * "Services in this area" (Map Phase 2): grouped counts from the
             * MULK places table — never a live external lookup. The gates are
             * the map layer's own public-pin contract: published, public,
             * duplicate-primary, operating. Zero-count groups are simply
             * absent, so the page never renders an empty shelf.
             */
            'services' => $this->serviceSummary($area),
    ...
    /**
     * Grouped service counts for one area, from the places database.
     *
     * Direct area assignment only (`area_id = this area`), matching how the
     * Places list below it is scoped — a count of 8 schools above a list
     * that can show those same 8 is a number a visitor can reconcile. The
     * grouping key is the admin-authoritative `place_categories.group`
     * column, ordered by the product's fixed group order with any
     * admin-invented group after them.
     *
     * @return list<array{key: string, label: string, count: int, categories: list<array{key: string, label: string, count: int}>}>
     */
    private function serviceSummary(Area $area): array
    {
        $counts = Place::query()
            ->published()
            ->where('is_public', true)
        ...
        foreach ($categories as $category) {
            ...
            $groups[$group] ??= ['key' => $group, 'label' => $this->groupLabel($group), 'count' => 0, 'categories' => []];
